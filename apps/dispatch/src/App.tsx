...
import { DispatchConsole } from "./DispatchConsole.js";

export function App() {
  const [authHeader, setAuthHeader] = useState<string | null>(
    () => sessionStorage.getItem("dispatch_auth")
  );

  function handleLogin(auth: string) {
    sessionStorage.setItem("dispatch_auth", auth);
    setAuthHeader(auth);
  }

  function handleLogout() {
    sessionStorage.removeItem("dispatch_auth");
    setAuthHeader(null);
  }

  if (!authHeader) return <LoginScreen onLogin={handleLogin} />;
  return <DispatchConsole authHeader={authHeader} onLogout={handleLogout} />;
}
...
function LoginScreen({ onLogin }: { onLogin: (auth: string) => void }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res  = await fetch(`${REST_BASE}/session`, { headers: { Authorization: auth } });
      ...
      <div style={{ width: "100%", maxWidth: 360 }}>
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.5rem" }}>
            ...
            <span style={{ fontWeight: 700, fontSize: "1.125rem", color: C.text }}>Dispatch Console</span>
          </div>
          <p style={{ color: C.muted, fontSize: "0.8125rem" }}>Sign in with your OpenMRS credentials</p>
        </div>
        ...
        <form onSubmit={(e) => void handleSubmit(e)} style={{
          ...
          <div style={{ marginBottom: "1rem" }}>
            ...
            <input
              type="text" autoCapitalize="off" autoComplete="username"
          ...
          <button
            ...
            style={{
              ...
              fontFamily: FONT, letterSpacing: "0.02em", transition: "background 0.15s",
            }}
          >
...
const inputStyle: React.CSSProperties = {
  ...
  borderRadius: 6, padding: "0.625rem 0.75rem",
  color: "#f1f5f9", fontFamily: FONT, fontSize: "0.9375rem",
